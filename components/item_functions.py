...
def heal(*args, **kwargs):
    ...
    if entity.fighter.hp == entity.fighter.max_hp:
        results.append({'consumed': False, 'message': Message('You are already at full health', colors.get('green'))})
    else:
...
def cast_fireball(*args, **kwargs):
    ...
    results.append({'consumed': True, 'message': Message('The fireball explodes, burning everything within {0} tiles!'.format(radius), colors.get('orange'))})
