...
def place_entities(room, entities, max_monters_per_room, max_items_per_room, colors):
    # Get a random number of monsters
    number_of_monters = randint(0, max_monters_per_room)
    # Get a random number of items
    number_of_items = randint(0, max_items_per_room)

    for i in range(number_of_monters):
        ...
        if not any([entity for entity in entities if entity.x == x and entity.y == y]):
            if randint(0, 100) < 80:
                ...
                monster = Entity(x, y, 'o', colors.get('desatured_green'), 'Orc', blocks=True, render_order=RenderOrder.ACTOR, fighter=fighter_component, ai=ai_component)
            else:
    ...
    for i in range(number_of_items):
        ...
        if not any([entity for entity in entities if entity.x == x and entity.y == y]):
            ...
            if item_chance < 70:
                ...
                item_component = Item(use_function=cast_fireball, targeting=True, targeting_message=Message('Left-click a target tile for the fireball, or right-click to cancel', colors.get('light_cyan')), damage=12, radius=3)
                item = Entity(x, y, '#', colors.get('red'), 'Fireball Scroll', render_order=RenderOrder.ITEM, item=item_component)
            elif item_chance < 90:
                item_component = Item(use_function=cast_confuse, targeting=True, targeting_message=Message('Left-click an enemy to confuse it, or right-click to cancel', colors.get('light_pink')))
...
def make_map(game_map, max_rooms, room_min_size, room_max_size, map_width, map_height, player, entities, max_monsters_per_room, max_items_per_room, colors):
    ...
    for r in range(max_rooms):
        ...
        # random position without goint out of the boundaries of the map
        ...
        for other_room in rooms:
            ...
            # center coordintates of the new room, will be useful later
            ...
            if num_rooms == 0:
                ...
                if randint(0, 1) == 1:
                    # fist move horizontally, then vertically
    ...
    staris_component = Stairs(game_map.dungeon_level + 1)
    down_stairs = Entity(center_of_last_room_x, center_of_last_room_y, '>', (255, 255, 255), 'Stairs', render_order=RenderOrder.STAIRS, stairs= staris_component)
...
def next_floor(player, message_log, dungeon_level, constants):
    ...
    make_map(game_map, constants['max_rooms'], constants['room_min_size'], constants['room_max_size'], constants['map_width'], constants['map_height'], player, entities, constants['max_monsters_per_room'], constants['max_items_per_room'], constants['colors'])
