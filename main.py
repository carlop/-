    ...
    while not tdl.event.is_window_closed():
        ...
        for player_turn_result in player_turn_results:
            ...
            if xp:
                ...
                message_log.add_message(Message('You gain {0} experiencepoints.'.format(xp)))

                if leveled_up:
                    message_log.add_message(Message('Your battle skills grow stronger! Your reached level {0}'.format(player.level.current_level) + '!', constants['colors'].get('yellow')))
                    previous_game_state = game_state
